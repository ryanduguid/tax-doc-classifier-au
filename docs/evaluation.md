...
| Optimisation regression cases | 14 | 14 | 5 |
...
The separate optimisation set was added after reproducing layout failures. Its 14 cases test payment instructions, longer address blocks, misleading requests, late conflicting headings and line-level OCR quality. It was used during implementation and is not held out. The original 57 fixtures remain unchanged. Evaluation now fails if any case is incorrect, an original dataset loses a category, or a dataset drops below its minimum size. Tests verify that a lower score fails independently of the saved report.
