...
| Optimisation regression cases | 20 | 20 | 8 |
...
The separate optimisation set was added after reproducing layout failures. Its 20 cases test payment instructions, longer address blocks, misleading requests, late conflicting headings and line-level OCR quality. PR review added positive controls for remittance and payment details, plus requests and instructions beyond line 32. It was used during implementation and is not held out. The original 57 fixtures remain unchanged. Evaluation now fails if any case is incorrect, an original dataset loses a category, or a dataset drops below its minimum size. Tests verify that a lower score fails independently of the saved report.
