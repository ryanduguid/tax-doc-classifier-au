...
A separate 32-case regression set covers address blocks, payment instructions, late mixed headings and unreliable OCR lines. All 32 pass. Evaluation fails on any incorrect prediction, missing category in the original datasets or reduction below their minimum case counts. Updating the saved results does not bypass these gates. New cases are development regressions, not an independent holdout.
