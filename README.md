...
Install the optional Python dependency in a dedicated environment:
...
The limits are 50 MiB per input, 500 extracted pages, 200,000 characters per page and 120 seconds per PDF extraction. Page-count validation happens after extraction.
...
The PDF check requires the offline runtime and exercises native pages, image-only refusal, OCR, empty pages and the built CLI. CI checks TypeScript, unit tests, builds and text evaluation on Windows and Linux; external OCR is not provisioned there.
